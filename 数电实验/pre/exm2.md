...
![](exm2photo/IMG_20171010_205958.jpg)
...
![](exm2photo/IMG_20171010_210015.jpg)
又由示波器的示数可得，振荡频率为4.434MHz。可以看到，由于晶振的选频特性极佳，所以振荡频率与标称值相同。

2. 观察晶体振荡器的频率稳定度<br/>
结果波形如图，其中，$R_0=2.14k\Omega,R_1=9.85M\Omega$
![](exm2photo/IMG_20171010_210018.jpg)
又由示波器示数可得，振荡频率为4.434MHz。也是由于晶振的选频特性，导致即使使用不同的电路结构，也还是能得到一样的振荡频率的结果。

## 思考题
3. 晶体振荡器的输出波形不是简谐波，是否会影响频率稳定性<br/>
答:会有很小的影响。若晶振有多个谐振频率，而刚好输出的非简谐波中相关的频率分离大小合适，则有可能使得整个系统振荡在一个不需要的频率。但是，又由于晶振选频能力非常强，所以一般情况下不会发生按照错误频率振荡的情况。
4. 非门与通常的反向放大器比较，有何相同之处？有何不同之处？<br/>
答:相同之处，均可表示非的逻辑，在要求不是非常严格的情况下甚至可以互换。<br/>
不同之处，反向放大器主要考虑的是输入与输出的线性关系，而非门则主要用于在输入超过阈值后改变输出。反向放大器后级的输出情况比较复杂，可能会接门电路，也可能接普通的模拟电路;非门的输出情况就没那么复杂，一般情况都是输出到下一级的门电路。
...
## 实验数据记录
![](exm2photo/IMG_20171010_212006.jpg)
![](exm2photo/IMG_20171010_212021.jpg)
